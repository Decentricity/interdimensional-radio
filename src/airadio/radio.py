...
from airadio import music3, song_title
...
class Config:
    def __init__(self, home: Path | None = None) -> None:
        ...
        self.lyrics_template = prompts_dir() / "normie-control.lyrics.template.txt"
...
def acquire_radio_lock(cfg: Config) -> None:
    ...
    handle = open(cfg.lock_file, "w", encoding="utf-8")
    try:
...
class SongGenerator:
    ...
    def _run(self) -> None:
        ...
        lyrics_path.write_text(
            song_title.build_lyrics(title, instrumental=instrumental), encoding="utf-8"
        )
        try:
            music3.generate(
                lyrics=lyrics_path,
        ...
        try:
            ...
            self._path = out
            archive_song(cfg, out, seed=seed, title=title)
...
def load_catalog(cfg: Config) -> dict:
    if cfg.catalog.is_file():
        return json.loads(cfg.catalog.read_text(encoding="utf-8"))
    return {"songs": []}
...
def ensure_catalog_hashes(cfg: Config) -> None:
    with _catalog_lock:
        ...
        for entry in catalog.get("songs", []):
            ...
            changed = True
        if changed:
            cfg.catalog.write_text(json.dumps(catalog, indent=2) + "\n", encoding="utf-8")


def archive_song(
    cfg: Config, src: Path, seed: int | None = None, title: str | None = None
) -> Path:
    ...
    with _catalog_lock:
        ...
        if title is not None and seed is not None:
            dest = cfg.library / song_title.song_filename(title, seed)
...
def import_staging_songs(cfg: Config) -> int:
    ...
    for src in sorted(cfg.staging.glob("pending-*.wav")):
        if not _valid_duration(src):
            continue
        if find_library_song(cfg, file_hash(src)) is not None:
            continue
